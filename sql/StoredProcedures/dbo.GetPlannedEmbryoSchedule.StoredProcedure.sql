...
/****** Object:  StoredProcedure [dbo].[GetPlannedEmbryoSchedule]    Script Date: 3/9/2020 7:27:08 PM ******/
DROP PROCEDURE IF EXISTS [dbo].[GetPlannedEmbryoSchedule]
GO
/****** Object:  StoredProcedure [dbo].[GetPlannedEmbryoSchedule]    Script Date: 3/9/2020 7:27:10 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetPlannedEmbryoSchedule]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[GetPlannedEmbryoSchedule] AS' 
END
GO



ALTER proc [dbo].[GetPlannedEmbryoSchedule] @UserID varchar(255), @StartDate date, @EndDate date, @ContractTypeID int
as
...
set nocount on
	...
	begin
		...
		begin
			...
			e.FarmColumn{NN}_Qty = SellableEggs, 
