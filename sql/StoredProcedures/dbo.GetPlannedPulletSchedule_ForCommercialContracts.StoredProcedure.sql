/****** Object:  StoredProcedure [dbo].[GetPlannedPulletSchedule_ForCommercialContracts]    Script Date: 3/9/2020 7:27:08 PM ******/
DROP PROCEDURE IF EXISTS [dbo].[GetPlannedPulletSchedule_ForCommercialContracts]
GO
/****** Object:  StoredProcedure [dbo].[GetPlannedPulletSchedule_ForCommercialContracts]    Script Date: 3/9/2020 7:27:10 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetPlannedPulletSchedule_ForCommercialContracts]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[GetPlannedPulletSchedule_ForCommercialContracts] AS' 
END
GO



ALTER proc [dbo].[GetPlannedPulletSchedule_ForCommercialContracts]	@UserID varchar(255), @StartDate date, @EndDate date, @ContractTypeID int
as
...
set nocount off
	...
	(
		...
		AvgSellableEmbryosPerBirdPerWeek numeric(8,6),
			...
			inner join 
				(
					select 
						...
						WeekEndingDate,
						CalcCommercialEggs = CalcCommercialEggs,
	...
	update ps set ps.TotalSellableEggsForWeek = ws.CalcCommercialEggs, ps.TotalPulletQty = ws.TotalPulletQty
	from #Data ps
	inner join 
	(
		select WeekEndingDate, sum(CalcCommercialEggs) as CalcCommercialEggs, sum(CalcPulletQty) as TotalPulletQty
		from RollingWeeklySchedule pfp
		where ContractTypeID = @ContractTypeID
		and exists (select 1 from #Data where WeekEndingDate = pfp.WeekEndingDate)
	...
	inner join 
	(
		select WeekEndingDate, AvgSellableEmbryosPerBirdPerWeek = avg(AvgSellableEmbryosPerBirdPerWeek)
		from #Data
		where TotalPulletQty <> 0
		group by WeekEndingDate
