...
Public Class csbAngular_GetListContentForScreen : Implements IHttpHandler, IReadOnlySessionState
    ...
    Dim _currentTabIndex As String
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        _currentTabIndex = context.Request.QueryString("currentTabIndex")
        ...
        buildHtmlContent(headerContent, advancedFindContent, rowContent, listContent, context)
        ...
        If Not isCalendar Then
            ...
            Dim sectionNodes As XmlNodeList = _xmlFormLayout.SelectNodes("fieldList/section")
            Dim rowClass As String = ""
            If sectionNodes(0).Attributes("rowClass") IsNot Nothing AndAlso sectionNodes(0).Attributes("rowClass").Value.ToString() > "" Then
                rowClass = "{{thisRecord." & sectionNodes(0).Attributes("rowClass").Value.ToString() & "}}"
            End If

            If isListTemplate Then
                context.Response.Write("<tr table-content-main-row=""table-content-main-row"" dir-paginate=""thisRecord in data.records | filter: data.compositeFilter | orderBy: data.sortString:data.sortReverse | itemsPerPage: data.pageSize "" current-page=""data.currentPage"" class=""" & rowClass & """>")
            Else
                context.Response.Write("<tr table-content-row=""table-content-row"" screen-id=""" & sScreenID & """ table-screen-id=""" & sScreenID & """ pagination-id=""" & sScreenID & """ dir-paginate=""thisRecord in theseRecords | filter: data.compositeFilterDetail('" & sScreenID.Trim() & "') | orderBy: theseRecords.sortString:theseRecords.sortReverse | itemsPerPage: data.pageSize "" current-page=""theseRecords.currentPage"" class=""" & rowClass & """>")
            End If
    ...
    Private Sub buildHtmlContent(ByRef headerContent As StringBuilder, ByRef advancedFindContent As StringBuilder, ByRef rowContent As StringBuilder, ByRef listContent As StringBuilder, context As HttpContext)
        ...
        For Each sectionNode In sectionNodes
            ...
            For Each fieldNode In sectionNode.ChildNodes
                ...
                'If the field references a screen id, check access to that screen. If no access, don't render the field
                If thisFP.ScreenID > "" Then
                    Dim fieldSI As ScreenItem = Common.ScreenReader.Item(thisFP.ScreenID, _userName)
                    If Not fieldSI Is Nothing Then
                        fieldSI.CheckAccess(context.Server.MapPath(""), context)
                        If Not fieldSI.AllowAccess Then
                            Continue For
                        End If
                    Else
                        Continue For
                ...
                Dim fieldNameForSorting As String = thisFP.FieldName
                If thisFP.FieldType.StartsWith("link") Then
                    If thisFP.FieldType.Substring(5).StartsWith("display") Then
                        fieldNameForSorting = thisFP.FieldType.Remove(0, 13)
                    End If
                End If
                ...
                If Not _thisSchema Is Nothing Then
                    ...
                    thisSchemaRows = _thisSchema.Select("ColumnName='" & fieldNameForSorting & "'")
                    If thisSchemaRows.Length > 0 Then
                        thisSchemaRow = _thisSchema.Select("ColumnName='" & fieldNameForSorting & "'")(0)
                ...
                Dim rawDisplayName As String = thisFP.DisplayName
                Dim displayName As String = thisFP.DisplayName
                Dim isDataFieldHeader As Boolean = False
                If displayName.ToLower().StartsWith("fieldname") Or displayName.ToLower().StartsWith("datafield") Then
                    isDataFieldHeader = True
                    rawDisplayName = _fieldDataHeaderPrefix & "." & displayName.Replace("fieldname ", "").Replace("fieldName ", "").Replace("datafield ", "").Replace("dataField ", "") & " | unsafe"
                    displayName = "{{" & rawDisplayName & "}}"
                End If

                headerContent.Append("<div ng-click=""" & dataPrefix & ".setColumnSorting(" & dataPrefix & ", '")
                headerContent.Append(fieldNameForSorting)
                headerContent.Append("','" & thisSchemaRowDataType & "');"" tabindex=""-1""")
                If isDataFieldHeader Then
                    headerContent.Append("ng-bind-html=""" & rawDisplayName & """")
                End If
                headerContent.Append(">")

                If Not isDataFieldHeader Then
                ...
                If Not thisFP.FieldType.ToLower().StartsWith("link") And Not thisFP.FieldType.ToLower().StartsWith("command") And Not excludeFromFind Then
                    ...
                    advancedFindContent.Append(Common.JSON_Encode(displayName).Replace("'", "&quot;"))
                ...
                ElseIf thisFP.ColumnNgShow > "" Then
                    rowContent.Append(" ng-show=""" & thisFP.ColumnNgShow & """ ")
                End If
                If thisFP.ColumnNgIf > "" Then
                    rowContent.Append(" ng-if=""" & thisFP.ColumnNgIf & """ ")
                End If
                ...
                ElseIf thisFP.ColumnNgShow > "" Then
                    listContent.Append(" ng-show=""" & thisFP.ColumnNgShow & """ ")
                End If
                If thisFP.ColumnNgIf > "" Then
                    listContent.Append(" ng-if=""" & thisFP.ColumnNgIf & """ ")
                End If
        ...
        If Not isListTemplate Then
            advancedFindContent.Append(", " & _currentTabIndex)
        End If
