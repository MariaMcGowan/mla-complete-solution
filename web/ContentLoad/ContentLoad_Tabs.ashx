    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If context.Request.QueryString("ScreenID") > "" Then
            ...
            If Not detailReferences Is Nothing Then
                ...
                '
                If Not detailReferences Is Nothing AndAlso detailReferences.Count > 0 Then

                    context.Response.Write("<md-tabs md-selected=""data.currentTabIndex"" md-dynamic-height md-border-bottom class=""md-whiteframe-z2 md-tabs-csb"">")
                    ...
                    For Each detailReference In detailReferences
                        If Not detailReference.DisplayStyle = "NoTab" Then
                            detailReference.CheckAccess(context.Server.MapPath(""), context)
                            If detailReference.AllowAccess Then
                                ...
                                If detailReference.NgIf > "" Then
                                    context.Response.Write("ng-if=""")
                                    context.Response.Write(detailReference.NgIf)
                                    context.Response.Write("""")
                                End If
                                ...
                                If detailReference.Count = "none" Then
                                    'Hide filter
                                Else
                                    context.Response.Write(" ({{(data.detailRecords[0].")
                                    context.Response.Write(detailReference.ID.ToString())
                                    If detailReference.Count Is Nothing OrElse detailReference.Count.Substring(0, 1) = "-" OrElse detailReference.Count.Substring(0, 1) = "+" Then
                                        context.Response.Write(".length)")
                                        If detailReference.Count IsNot Nothing Then
                                            context.Response.Write(detailReference.Count)
                                        End If
                                    Else
                                        context.Response.Write(" | filter:")
                                        context.Response.Write(detailReference.Count)
                                        context.Response.Write(":true).length")
                                    End If
                                    context.Response.Write("}})")
                                End If
                                ...
                                context.Response.Write("<md-content layout-padding>")

                                context.Response.Write("<form-tab-content current-tab-index='" & currentTabIndex.ToString() & "' current-tab-display-name='" & detailReference.DisplayName.Replace("""", """""").Replace("'", "''").Replace(" ", "_") & "' data='data' this-record='data.detailRecords[0].")
                                context.Response.Write(detailReference.ID.ToString())
