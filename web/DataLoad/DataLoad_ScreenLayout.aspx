...
<script language="VB" runat="server">
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        If mainXmlFormLayoutFileName.EndsWith(".xml") Then
            ...
            End If
            If Not listSectionNodes(0) Is Nothing AndAlso Not listSectionNodes(0).Attributes("formClassName") Is Nothing Then
                mainClass = listSectionNodes(0).Attributes("formClassName").Value.ToString()
            End If
        End If
        ...
        If mainXmlFormLayoutFileName.EndsWith(".xml") Then
            ...
            Next
            If Not mainSectionNodes(0) Is Nothing AndAlso Not mainSectionNodes(0).Attributes("formClassName") Is Nothing Then
                findClass = mainSectionNodes(0).Attributes("formClassName").Value.ToString()
            End If
        End If
        ...
        If Not listScreenItem.DisplayStyle Is Nothing Then
            ...
            If (listScreenItem.DisplayStyle.ToLower().Contains("hidepaging") AndAlso CType(listScreenItem.DisplayStyle.Substring(listScreenItem.DisplayStyle.IndexOf("hidepaging") + 12, 4), Boolean)) Then
                hidePaging = "true"
            End If
            If (listScreenItem.DisplayStyle.ToLower().Contains("hidesearch") AndAlso CType(listScreenItem.DisplayStyle.Substring(listScreenItem.DisplayStyle.IndexOf("hidesearch") + 12, 4), Boolean)) Then
                hideSearch = "true"
            End If
        End If
        Response.Write(", ""hidePaging"": """ & hidePaging & """")
        Response.Write(", ""hideSearch"": """ & hideSearch & """")
        Response.Write(", ""findClass"": """ & findClass & """")
        Response.Write(", ""mainClass"": """ & mainClass & """")
        Response.Write(", ""sectionDisplayStyle"": """ & sectionDisplayStyle & """")

        Dim fixedHeader As String = "false"
        If sectionDisplayStyle.ToLower().Contains("fixedheader") AndAlso sectionDisplayStyle.Substring(sectionDisplayStyle.ToLower().IndexOf("fixedheader") + 12, 4).ToLower() = "true" Then
            fixedHeader = "true"
        End If
        Response.Write(", ""fixedHeader"": " & fixedHeader & "")
        ...
        If Not reportRef Is Nothing AndAlso reportRef.Count > 0 Then
            ...
            Response.Write(", ""reportUrl"": """ & Common.JSON_Encode(BuildReportUrl(reportRef.Item(0), False)) & """")
        ...
        If Not redirect Is Nothing AndAlso redirect.Count > 0 Then
            ...
            If Not redirect.Item(0).LinkFields Is Nothing AndAlso redirect.Item(0).LinkFields.Count > 0 Then
                For Each linkField As String In redirect.Item(0).LinkFields
                    If currentParam = "" Then
                        currentParam = "{{thisRecord." + linkField + "}}"
                    Else
                        currentParam = currentParam + "," + "{{thisRecord." + linkField + "}}"
                    End If
                Next
            End If
            If currentParam = "" Then
                currentParam = sParameter
            End If
            Response.Write(", ""redirect"": ")
            Response.Write("""" + Common.JSON_Encode(Common.ScreenReader.RenderHREF(redirect.Item(0).ID, False, currentParam.Replace(",", "&p="), Common.UserName(Context))) + """")
        End If
        ...
        'ScreenItemList should still be details or empty. We need to process the order they are saved in.
        'We need to add left nav to detail LeftNav
        Dim leftNavList As System.Collections.Generic.List(Of csi.Framework.ScreenData.ScreenItem) = csi.Framework.Utility.Common.ScreenReader.References(sScreenID, "LeftNav", Common.UserName(Context))
        If Not leftNavList Is Nothing AndAlso leftNavList.Count > 0 Then
            If ScreenItemList IsNot Nothing Then
                For Each csiSI In leftNavList
                    ScreenItemList.Add(csiSI)
                Next
            Else
                ScreenItemList = leftNavList
            End If
        End If
        If Not ScreenItemList Is Nothing AndAlso ScreenItemList.Count > 0 Then
            ...
            Response.Write(", ""detailSearchComparators"" : {")
            Dim searchComparatorList As List(Of String) = New List(Of String)
            For Each csiSI In ScreenItemList
                searchComparatorList.Add("""" & csiSI.ID & """ : """ & csi.Framework.Utility.Common.get_search_comparator(csiSI) & """")
            Next
            Response.Write(String.Join(",", searchComparatorList))
            Response.Write("}")
        ...
        Dim overrideLeftMenu As Boolean = False
        ...
        If Not ScreenItemList Is Nothing Then
            ScreenItemList(0).CheckAccess("", Context)
        End If
        ...
        Else
            Response.Write("false")
        End If
        Response.Write(", ""overrideLeftMenu"": ")
        If overrideLeftMenu Then
            Response.Write("true")
        Else
            Response.Write("false")
        End If
        ...
        writeAdditionalActions(sScreenID, sParameter, isForm, ", ""additionalActions"": [")

        'add additional processes for each detail reference
        ScreenItemList = csi.Framework.Utility.Common.ScreenReader.References(sScreenID, "detail", Common.UserName(Context))
        If Not ScreenItemList Is Nothing AndAlso ScreenItemList.Count > 0 Then
            For Each detailItem As ScreenItem In ScreenItemList
                writeAdditionalActions(detailItem.ID, sParameter, isForm, ", """ & detailItem.ID & """: [")
            Next
        End If
        ...
        ' add the search comparator for the main screen
        Response.Write(", ""searchComparator"":""" & csi.Framework.Utility.Common.get_search_comparator(mainScreenItem) & """")

        'Let's add the Application Variables
    ...
    Private Sub writeAdditionalActions(sScreenID As String, sParameter As String, isForm As Boolean, prefix As String)
        Dim additionalActionFirst As Boolean = True
        Dim ScreenItemList = New System.Collections.Generic.List(Of csi.Framework.ScreenData.ScreenItem)
        ...
        If Not ScreenItemList Is Nothing AndAlso ScreenItemList.Count > 0 Then
            ...
            For Each csiSI In ScreenItemList
                ...
                End If
                Dim headerLinkURL As String = Common.JSON_Encode(Common.ScreenReader.RenderHREF(csiSI.ID, False, currentParameter.Replace(",", "&p="), Common.UserName(Context)))
                Dim linkExt As String = csiSI.PageName.Substring(csiSI.PageName.LastIndexOf(".") + 1, csiSI.PageName.Length - csiSI.PageName.LastIndexOf(".") - 1)
                If Not ConfigurationManager.AppSettings.Item(linkExt) Is Nothing Or csiSI.PageName.ToLower().EndsWith(".rpx") Then
                    headerLinkURL = Common.JSON_Encode(BuildReportUrl(csiSI, True) & "&p=" & currentParameter.Replace(",", "&p="))
                End If
                If headerLinkURL > "" Then
                    ...
                    ElseIf csiSI.ItemType = "html" Then
                        Response.Write(" ,""action"": ""html""")
                    Else
                    ...
                    If csiSI.ConfirmationMessage IsNot Nothing AndAlso csiSI.ConfirmationMessage.Length > 0 Then
                        ...
                        Response.Write("""")
                    End If
                    If csiSI.AccessKey > "" Then
                        Response.Write(" ,""AccessKey"": """)
                        Response.Write(csiSI.AccessKey)
                        Response.Write("""")
                    End If
                    ...
                    If Not csiSI.DisplayStyle Is Nothing Then
                        Dim linkImageName = csiSI.DisplayStyle.Replace("image:", "").Replace("icon:", "")
                        Dim displayStyle = ""
                        If csiSI.DisplayStyle.StartsWith("icon:") Then
                            displayStyle = "icon:"
                        ElseIf csiSI.DisplayStyle.StartsWith("image:") Then
                            displayStyle = "image:"
                        End If
                        If linkImageName.StartsWith("field") Then
                            displayStyle = displayStyle & "{{data.record." & linkImageName.Substring(6) & "}}"
                        ElseIf Not ConfigurationManager.AppSettings(linkImageName) Is Nothing Then
                            displayStyle = displayStyle & ConfigurationManager.AppSettings(linkImageName)
                        Else
                            displayStyle = displayStyle & linkImageName
                        End If

                        Response.Write(Common.JSON_Encode(displayStyle, True))
                    End If
                    Response.Write("""")
    ...
    Private Function BuildReportUrl(csiSI As ScreenItem, renderLink As Boolean) As String
        ...
        If csiSI.DisplayStyle > "" And csiSI.DisplayStyle <> "always visible" And csiSI.DisplayStyle <> "popup" And Not renderLink Then
        ...
        If renderLink Then
            If csiSI.reportFormat > "" Then
                sURL.Append("&ReportFormat=" & csiSI.reportFormat)
            Else
                sURL.Append("&ReportFormat=PDF")
            End If
