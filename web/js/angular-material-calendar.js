...
angular.module("materialCalendar").directive("calendarMd", ["$compile", "$parse", "$templateRequest", "$q", "materialCalendar.Calendar", "MaterialCalendarData", function ($compile, $parse, $templateRequest, $q, Calendar, CalendarData) {
    ...
    return {
        ...
        link: function ($scope, $element, $attrs) {
            ...
            $scope.contentType = $attrs.contentType || 'form';
            $scope.dateField = $attrs.dateField;
            ...
            $scope.sameMonth = function (date) {
                ...
                var d = angular.copy(calcDate);
                return d.getFullYear() === $scope.calendar.year &&
                    d.getMonth() === $scope.calendar.month;
            };
            //$scope.sameDate = function (calendarDate, sqlDate) {
            //    var calc2 = new Date(sqlDate);
            //    var calc1 = new Date(calendarDate);
            //    return calc1 - calc2 == 0;
            //};
            $scope.sameDate = function (day, fieldName) {
                calcDate1 = new Date(day);
                return function (thisRecord) {
                    calcDate2 = new Date(thisRecord[fieldName]);
                    return calcDate1 - calcDate2 == 0;
                };
            };
            ...
            $scope.handleDayClick = function (date) {
                ...
                var active = angular.copy($scope.active);
                //added to use shift key instead of configuring multiselect or single select
                if (shiftKeyDown && !angular.isArray(active)) {
                    active = [active];
                } else if (!shiftKeyDown && angular.isArray(active)) {
                    active = false;
                }
                if (angular.isArray(active)) {
            ...
            var init = function () {
                ...
                //    //FORM repeats per record
                //    + "<div ng-repeat=\"thisRecord in records | filter:sameDate(day,'" + $scope.dateField + "')\" style=\"width:100%;\">"
                //    + templateResponse
                //    + "</div>"
                //    + "</div>"
                //    + "</div>"
                ...
                //    + "<div class=\"calendarDayContent\" ng-repeat=\"thisRecord in records | filter:sameDate(day,'" + $scope.dateField + "')\" style=\"width:100%;\">"
                //    + "</div>"
                //    //end day
                //    + "</div>"
                //    //end week
